...
def user_data_dir() -> Path:
    ...
    override = os.environ.get("BOMIQ_DATA_DIR")
    if override:
    ...
    return Path(base) / "bomiq"


def user_config_dir() -> Path:
    override = os.environ.get("BOMIQ_CONFIG_DIR")
    if override:
        return Path(override).expanduser()
    system = platform.system()
    if system == "Windows":
...
MOUSER = _register(ProviderSpec(
    ...
    notes="Simple API key. Free tier is limited to ~1000 calls/day and 30 "
          "calls/minute, so BOM-IQ throttles to 1.5 req/s.",
))
...
class Config:
    ...
    def db_path(self) -> Path:
        return self.data_dir / "bomiq.sqlite3"
