...
def setup(log_dir: Path | None = None, level: str | int = "INFO",
    ...
    """Configure the ``schemata_bom`` logger. Safe to call repeatedly."""
    global _configured
    logger = logging.getLogger("schemata_bom")
    if _configured:
        return logger
    ...
    if log_dir is not None:
        try:
            ...
            file_handler = logging.handlers.RotatingFileHandler(
                log_dir / "schemata_bom.log", maxBytes=4_000_000, backupCount=5,
                encoding="utf-8",
            )
...
def get(name: str) -> logging.Logger:
    ...
    if not _configured:
        setup(console=os.environ.get("SCHEMATA_BOM_QUIET") != "1")
    return logging.getLogger(f"schemata_bom.{name}")
